...
router.post("/statistics", async (req, res) => {
    const { contaminante, contaminantes, regiones, estaciones, rangoFechas } = req.body;
    ...
    try {
        ...
        if (contaminante && typeof contaminante !== "string") {
            return res.status(400).send("'contaminante' debe ser un string");
        }
        ...
        if (contaminante) {
            filtros.push(`c.nombre = $${contador}`);
            valores.push(contaminante);
            contador++;
        } else if (contaminantes && contaminantes.length > 0) {
...
router.post("/variance-and-cv", async (req, res) => {
    const { contaminante, contaminantes } = req.body;
